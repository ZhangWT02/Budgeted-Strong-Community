...
// g++ -DMOVIELENS -O2 -o AB_Shrink_Movielens AB_Shrink.cpp
// ./AB_Shrink_Movielens 2 5 10 10 0.5
...
char* OUTPUT_FILE = (char*)"Output/Movielens_shrink-adaptive.txt";
enum VertexType {
    User,
    Age,
    Occupation,
...
char* OUTPUT_FILE = (char*)"Output/dblp_shrink-adaptive.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/foursquare_shrink-adaptive.txt";
enum VertexType {
    User,
    Venues,
    VenuesType
...
char* OUTPUT_FILE = (char*)"Output/amazon_shrink-adaptive.txt";
enum VertexType {
    User,
    Product,
    Brand,
...
char* OUTPUT_FILE = (char*)"Output/yago_shrink-adaptive.txt";
enum VertexType {
    Person,
    airport,
    imports_wordnet,
...
char* OUTPUT_FILE = (char*)"Output/dblp20_shrink-adaptive.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp40_shrink-adaptive.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp60_shrink-adaptive.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp80_shrink-adaptive.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp_case_shrink-adaptive.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
typedef unordered_map<VertexType, double> expect_threshold;
...
void printH() {
    ...
    for (int i = 0; i < Hsize; ++i) {
        ...
        if (g.v[vertexId].type == targetType) {
            ...
            for (auto p_neignbor : metaGraph[vertexId]) {
                outfile << vertexId << " " << p_neignbor.first << " " << p_neignbor.second << endl;
            }
...
void printParameter(){
    ...
    outfile << "Shrink Rate: " << shrink_rate << endl;
...
void delVertexAndUpdateQ(int id, vector<int>& Q, unordered_set<int>& isInQ) {
    ...
    for (int i = 0; i < g.v[id].deg; i++) {
        int neighbor = g.v[id].neighbors[i];
        if (isInQ.find(neighbor) == isInQ.end() && g.v[neighbor].deg == degreeThreshold[g.v[neighbor].type]) {
...
void adaptive_shrink() {
    ...
    while (true) {
        iteration_times++;
        int n_exp = ceil(float(target_vertex_number - size_constraint) * shrink_rate);
        if (n_exp < 10) {
            break;
        }
        unordered_map<VertexType, int> total_vertex;
        unordered_map<VertexType, int> total_edge;
        expect_threshold deg_exp;
        int sup_exp = 1;
        for (int i = 0; i < Hsize; ++i) {
            total_vertex[g.v[H[i]].type] += 1;
            total_edge[g.v[H[i]].type] += g.v[H[i]].deg;
        }
        for (auto outer_pair: total_edge) {
            deg_exp[outer_pair.first] = double(outer_pair.second) / total_vertex[outer_pair.first];
        }
        for (int i = 0; i < metaPathLength-1; i++) {
            sup_exp *= deg_exp[metaPath[i]];
        }

        // 每个pair存放:   <score, id>
        // score = (sum(sup) + deg) / distance^2
        vector<pair<double, int>> data;
        for (int i = 0; i < Hsize; i++) {
            if (g.v[H[i]].type == targetType
                && H[i] != queryVertexId) {
                if (g.v[H[i]].instanceNum < sup_exp || g.v[H[i]].deg < deg_exp[targetType]) {
                    // cout << sup+g.v[H[i]].deg << ", " << g.v[H[i]].distance << ", " << H[i] << endl;
                    if (g.v[H[i]].distance == 0) {
                        data.push_back({0, H[i]});
                    }
                    else {
                        data.push_back({double(g.v[H[i]].instanceNum) / pow(g.v[H[i]].distance, 2), H[i]});
                    }
                }
            }
        }
        ...
        if (data.size() > n_exp) {
            ...
            for (int i = 0; i < n_exp; i++) {
                Q.push_back(S.top().second);
                isInQ.insert(S.top().second);
                S.pop();
            }
        }
        else {
        ...
        while (!Q.empty()) {
            ...
            vector<int> tempQ = Q;
            for (int id : Q) {
                delVertexAndUpdateQ(id, tempQ, isInQ);
                delVertex(id);
            }
            vector<int> newQ(tempQ.begin() + Q.size(), tempQ.end());
            Q = newQ;
        }
        target_vertex_number -= shrink_stack[shrink_stack_size].delTargetNum;    
        
        auto it = find(H.begin(), H.begin() + Hsize, queryVertexId);
        if (it == H.begin() + Hsize) {
            // queryVertexId被删除，需要回溯并break while
            // cout << "queryVertex is deleted" << endl;
            // recover Hsize
            Hsize += shrink_stack[shrink_stack_size].delHsize;
            // cout << "after recover Hsize: " << Hsize << endl;
            // recover TargetNum
            target_vertex_number += shrink_stack[shrink_stack_size].delTargetNum;            
            // cout << "after recover target_vertex_number: " << target_vertex_number << endl;
            // recover degree
    ...
    while (true) {
        ...
        // 随机选，就选id最小的
        int id = -1;
        // score function: sup的和
        int sf = INT_MAX;
        for (int j = 0; j < Hsize; ++j) {
            if (g.v[H[j]].type == targetType 
                && shrink_stack[shrink_stack_size].isInCandidate.find(H[j]) == shrink_stack[shrink_stack_size].isInCandidate.end()) {
                // if (g.v[H[j]].distance == 0) {
                //     id = H[j];
                //     break;
                // }
                // else 
                if (g.v[H[j]].instanceNum < sf) {
                    id = H[j];
                    sf = g.v[H[j]].instanceNum;
                }
            }
        }
        // 如果当前所有Candidate都在H中，且，有可选择的节点
        if (shrink_stack[shrink_stack_size].Candidate.size() <= size_constraint && isPromingBranch() && id != -1) {
            ...
            shrink_stack[shrink_stack_size].delHsize = 0;
            shrink_stack[shrink_stack_size].delTargetNum = 0;
            shrink_stack[shrink_stack_size].select_vec = {selected_id};
            shrink_stack[shrink_stack_size].Candidate = shrink_stack[shrink_stack_size-1].Candidate;
            shrink_stack[shrink_stack_size].isInCandidate = shrink_stack[shrink_stack_size-1].isInCandidate;
        ...
        // 回溯
            ...
            back_track_times++;
            // cout << "back track when shrink_stack_size: " << shrink_stack_size << endl;
            if (shrink_stack_size == 0) {
                Hsize = 0;
                return ;
            }
            ...
            shrink_stack[shrink_stack_size-1].Candidate.push_back(shrink_stack[shrink_stack_size].select_vec[0]);
            shrink_stack[shrink_stack_size-1].isInCandidate.insert(shrink_stack[shrink_stack_size].select_vec[0]);
            shrink_stack[shrink_stack_size] = shrink_stack_object();
            --shrink_stack_size;
        }
...
//     }
//     shrink_stack.resize(target_vertex_number - size_constraint + 1);
//     shrink_stack[shrink_stack_size].CandidateSetSize = 1;

//     while (target_vertex_number > size_constraint) {
//         cout << "target_vertex_number: " << target_vertex_number << endl;
//         // if promising branch -> delete
//         // else back track
//         // 随机选，就选id最小的
//         int i = 0;
//         while (g.v[H[i]].type != targetType || isValInVector(H[i], Candidate)) {
//             i++;
//         }
//         // 如果当前所有Candidate都在H中，且，有可选择的节点
//         if (isPromingBranch() && i < Hsize) {
//             cout << "select " << H[i] << " to delete when shrink_stack_size: " << shrink_stack_size << endl;
//             int selected_id = H[i];
//             // 删除v，并将影响记录在stack中
//             shrink_stack_size++;
//             shrink_stack[shrink_stack_size].selectedVertexId = selected_id;
//             shrink_stack[shrink_stack_size].CandidateSetSize = shrink_stack[shrink_stack_size-1].CandidateSetSize;
//             vector<int> Q = {selected_id};
//             while (!Q.empty()) {
//                 int id = Q.back();
//                 Q.pop_back();
...
//             }
//             // 更新下一轮迭代所需的CandidateSet
//             if (Candidate.size() == shrink_stack[shrink_stack_size-1].CandidateSetSize) {
//                 Candidate.push_back(shrink_stack[shrink_stack_size].selectedVertexId);
//             }
//             else {
//                 Candidate[shrink_stack[shrink_stack_size].CandidateSetSize] = shrink_stack[shrink_stack_size].selectedVertexId;
//             }
//             --shrink_stack_size;
//             shrink_stack[shrink_stack_size].CandidateSetSize += 1;
//         }
...
void updateDistance() {
    ...
    stack.push_back({queryVertexId, 1});
    visited[queryVertexId] = true;
    while(!stack.empty()) {
        auto vec = stack.back();
        int vertexId = vec[0];
        int dis = vec[1];
        g.v[vertexId].distance = dis;
        stack.pop_back();
        for (auto pair: metaGraph[vertexId]) {
            int p_neighbor = pair.first;
            if (!visited[p_neighbor]) {
                stack.push_back({p_neighbor, dis+1});
                visited[p_neighbor] = true;
            }
        }
    }
}
