...
// g++ -DMOVIELENS -O2 -o BasicShrink_sf_Movielens BasicShrink_sf.cpp
// ./BasicShrink_sf_Movielens 14 2 5 10 10
...
char* OUTPUT_FILE = (char*)"Output/Movielens_shrink-sf.txt";
enum VertexType {
    User,
    Age,
    Occupation,
...
char* OUTPUT_FILE = (char*)"Output/dblp_shrink-sf.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/foursquare_shrink-sf.txt";
enum VertexType {
    User,
    Venues,
    VenuesType
...
char* OUTPUT_FILE = (char*)"Output/amazon_shrink-sf.txt";
enum VertexType {
    User,
    Product,
    Brand,
...
char* OUTPUT_FILE = (char*)"Output/yago_shrink-sf.txt";
enum VertexType {
    Person,
    airport,
    imports_wordnet,
...
char* OUTPUT_FILE = (char*)"Output/dblp20_shrink-sf.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp40_shrink-sf.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp60_shrink-sf.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp80_shrink-sf.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp_case_shrink-sf.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
typedef struct {
    ...
    unordered_map<int, int> delDegree;
    // shrinkStack[i].delMetaGraph: 删除后，MetaGraph变化量
    unordered_map<int, unordered_map<int, int>> delMetaGraph;
    // shrinkStack[i].delInstanceNum   删除后，instanceNum下降的节点：instanceNum下降了多少
    unordered_map<int, int> delInstanceNum;
...
void basic_shrink() {
    ...
    while (true) {
        ...
        if (shrink_stack[shrink_stack_size].Candidate.size() <= size_constraint && isPromingBranch() && id != -1) {
            int selected_id = id;
            // cout << "select " << selected_id << " to delete when shrink_stack_size: " << shrink_stack_size << endl;
            // 删除v，并将影响记录在stack中
            shrink_stack_size++;
