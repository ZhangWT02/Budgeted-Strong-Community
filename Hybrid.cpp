...
// g++ -DMOVIELENS -O2 -o Hybrid_Movielens Hybrid.cpp
// ./Hybrid_Movielens 2 5 10 10
...
char* OUTPUT_FILE = (char*)"Output/Movielens_hybrid.txt";
enum VertexType {
    User,
    Age,
    Occupation,
...
char* OUTPUT_FILE = (char*)"Output/dblp_hybrid.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/foursquare_hybrid.txt";
enum VertexType {
    User,
    Venues,
    VenuesType
...
char* OUTPUT_FILE = (char*)"Output/amazon_hybrid.txt";
enum VertexType {
    User,
    Product,
    Brand,
...
char* OUTPUT_FILE = (char*)"Output/yago_hybrid.txt";
enum VertexType {
    Person,
    airport,
    imports_wordnet,
...
char* OUTPUT_FILE = (char*)"Output/dblp20_hybrid.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp40_hybrid.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp60_hybrid.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp80_hybrid.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp_case_hybrid.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
// unordered_map<int, unordered_map<int, int>> add_subgraph_MetaGraph;
// unordered_map<int, int> add_subgraph_InstanceNum;
...
void delVertex_from_C(int id, vector<int>& Q, unordered_set<int>& isInQ);
...
void delVertex(int id) {
    ...
    if (it != H.begin() + Hsize) {
        ...
        if (g.v[id].type == targetType) {
            ...
            auto itt = find(Candidate.begin(), Candidate.end(), id);
            if (itt != Candidate.end()) {
                Candidate.erase(itt);
            }
        }
    }
...
void hybrid() {
    ...
    graph_C.push_back(queryVertexId);
    ...
    {
        iteration_times++;
        if (C.size() + Candidate.size() <= size_constraint) {
            end_time = chrono::high_resolution_clock::now();
            printParameter();
            printH();
            return ;
        }
        if (C.size() > size_constraint) {
            Csize = 0;
            end_time = chrono::high_resolution_clock::now();
            printParameter();
            printC();
        ...
        for (auto id : Candidate) {
            ...
            for (auto pair : metaGraph[id]) {
                score_all += pair.second;
                if (pair.second > k && isValInVector(pair.first, Candidate)) {
                    score_C += pair.second;
                }
            }
            score_all += score_C;
            avg_score += score_all;
            score.push_back(score_all);
            // score function
            if (score_C > 0 && score_all > score_max) {
                score_max = score_all;
                max_id = id;
            }
            if (score_all < score_min) {
                score_min = score_all;
                min_id = id;
            }
        }
        avg_score = avg_score / Candidate.size();
        if (avg_score - score_min < score_max - avg_score) {
            // expand
            // cout << "expand" << endl;
            C.push_back(max_id);
            auto it = find(Candidate.begin(), Candidate.end(), max_id);
            if (it != Candidate.end()) {
                Candidate.erase(it);
            }
            g.v[max_id].status = 1;
            graph_C.push_back(max_id);
            Csize = graph_C.size();
            ...
            // cout << "shrink" << endl;
            vector<int> Q = {min_id};
            unordered_set<int> isInQ = {min_id};
            auto it = find(Candidate.begin(), Candidate.end(), min_id);
            if (it != Candidate.end()) {
                Candidate.erase(it);
            }
            while (!Q.empty()) {
    ...
    //         auto it = find(Candidate.begin(), Candidate.begin() + expand_stack[expand_stack_size-1].CandidateSize, expand_stack[expand_stack_size].selectedVertexId);
        ...
        if (stack.size() == metaPathLength) {
            ...
            if (g.v[stack.back()[0]].status == 1) {
                for (auto& pair : stack) {
                    ...
                    // add_subgraph_InstanceNum[pair[0]] += 2;
                    if (g.v[pair[0]].status == 0) {
                        g.v[pair[0]].status = 1;
                        graph_C.push_back(pair[0]);
                        Csize++;
                    }
                }
                subgraph_metaGraph[stack.front()[0]][stack.back()[0]] += 1;
                subgraph_metaGraph[stack.back()[0]][stack.front()[0]] += 1;
                // add_subgraph_MetaGraph[stack.front()[0]][stack.back()[0]] += 1;
                // add_subgraph_MetaGraph[stack.back()[0]][stack.front()[0]] += 1;
            }
...
void find_DPcore_from_C() {
    ...
    vector<int> origin_C = graph_C;
    for (int i = 0; i < graph_C.size(); i++) {
        int deg_count = 0;
        for(auto neighbor : g.v[graph_C[i]].neighbors) {
            if (g.v[neighbor].status == 1) {
                deg_count++;
            }
        }
        if (deg_count < degreeThreshold[g.v[graph_C[i]].type]) {
            Q.push_back(graph_C[i]);
            isInQ.insert(graph_C[i]);
        }
    }
    ...
    if (Csize == 0){
        ...
        // recover C & status
        graph_C = origin_C;
        for (int i = 0; i < origin_C.size(); i++) {
            g.v[graph_C[i]].status = 1;
        }
...
void delVertex_from_C(int id, vector<int>& Q, unordered_set<int>& isInQ) {
            ...
            while (!stack.empty()) {
                if (stack.size() == metaPathLength) {
                    ...
                    if (delIndex == metaPathLength-1) {
                        ...
                        if (Psupport_in_C(stack.back()[0]) < k && isInQ.find(stack.back()[0]) == isInQ.end() && stack.back()[0] != id) {
                            Q.push_back(stack.back()[0]);
                        }
                    }
                    else {
                        ...
                        if (Psupport_in_C(stack.front()[0]) < k && isInQ.find(stack.front()[0]) == isInQ.end() && stack.front()[0] != id) {
                            Q.push_back(stack.front()[0]);
                        }
                    }
                    ...
                    for (auto& pair : stack) {
                        ...
                        if (g.v[pair[0]].subgraph_instanceNum == 0 && isInQ.find(pair[0]) == isInQ.end() && pair[0] != id) {
                            Q.push_back(pair[0]);
                        }
                    }
            ...
            for (const auto& instance1 : instances1) {
                for (const auto& instance2 : instances2) {
                    ...
                    if (seen.size() == metaPathLength) {
                        ...
                        if (Psupport_in_C(sourceVertexId) < k && isInQ.find(sourceVertexId) == isInQ.end() && sourceVertexId != id) {
                            Q.push_back(sourceVertexId);
                        }
                        // update subgraph_instanceNum
                        for (const auto& i : seen) {
                            ...
                            if (g.v[i].subgraph_instanceNum == 0 && isInQ.find(i) == isInQ.end() && i != id) {
                                Q.push_back(i);
                            }
                        }
                    }
    ...
    auto it = find(graph_C.begin(), graph_C.begin() + Csize, id);
    if (it != graph_C.begin() + Csize) {
        swap(*it, graph_C[Csize - 1]);
        --Csize;
    }
    g.v[id].status = 0;
}
