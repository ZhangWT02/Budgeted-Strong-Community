...
// g++ -DMOVIELENS -O2 -o BasicExpand_Movielens BasicExpand.cpp
// g++ -DAMAZON -O2 -o BasicExpand_amazon BasicExpand.cpp
// ./BasicExpand_amazon 8995 5 2 2 10
...
char* OUTPUT_FILE = (char*)"Output/Movielens_expand-basic.txt";
enum VertexType {
    User,
    Age,
    Occupation,
...
char* OUTPUT_FILE = (char*)"Output/dblp_expand-basic.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/foursquare_expand-basic.txt";
enum VertexType {
    User,
    Venues,
    VenuesType
...
char* OUTPUT_FILE = (char*)"Output/amazon_expand-basic.txt";
enum VertexType {
    User,
    Product,
    Brand,
...
char* OUTPUT_FILE = (char*)"Output/yago_expand-basic.txt";
enum VertexType {
    Person,
    airport,
    imports_wordnet,
...
char* OUTPUT_FILE = (char*)"Output/dblp20_expand-basic.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp40_expand-basic.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp60_expand-basic.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp80_expand-basic.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/dblp_case_expand-basic.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
void basic_expand() {
    ...
    for (int i = 0; i < Hsize; ++i) {
        if (g.v[H[i]].type == targetType) {
            ...
            expand_stack[expand_stack_size].CandidateSet.push_back(H[i]);
        }
    }
    ...
    auto it = find(expand_stack[expand_stack_size].CandidateSet.begin(), expand_stack[expand_stack_size].CandidateSet.end(), queryVertexId);
    if (it != expand_stack[expand_stack_size].CandidateSet.end()) {
        expand_stack[expand_stack_size].CandidateSet.erase(it);
    }
    expand_stack[expand_stack_size].SelectSet = {queryVertexId};
    ...
    while (Csize == 0)
    {
        iteration_times++;
        int id = -1;
        for (int current_solution_vertexId : expand_stack[expand_stack_size].SelectSet) {
            for (auto pair : metaGraph[current_solution_vertexId]) {
                if (pair.second >= k && g.v[pair.first].status == 0) {
                    // TODO: 这里选择第一个扫描到的support大于等于k，且未在C中的节点
                    id = pair.first;
                    break;
                }
            }
        ...
        // 添加节点到C
        if (id != -1 
            && expand_stack[expand_stack_size].SelectSet.size() < size_constraint){
            ...
            expand_stack[expand_stack_size].SelectSet = expand_stack[expand_stack_size - 1].SelectSet;
            expand_stack[expand_stack_size].CandidateSet = expand_stack[expand_stack_size - 1].CandidateSet;

            auto it = find(expand_stack[expand_stack_size].CandidateSet.begin(), expand_stack[expand_stack_size].CandidateSet.end(), id);
            if (it != expand_stack[expand_stack_size].CandidateSet.end()) {
                expand_stack[expand_stack_size].CandidateSet.erase(it);
                expand_stack[expand_stack_size].SelectSet.push_back(id);
            }
            else {
                cout << "add vertex, id not in CandidateSet" << endl;
            }
        ...
        // 回溯
            ...
            int id = expand_stack[expand_stack_size].selectedVertexId;
            ...
            if (it != expand_stack[expand_stack_size].CandidateSet.end()) {
                ...
                cout << "back track, id not in CandidateSet" << endl;
            }
            for (int i : expand_stack[expand_stack_size].CandidateSet) {
                g.v[i].status = 0;
            }
            // for (int i = 0; i < expand_stack[expand_stack_size].SelectSize; i++) {
            //     g.v[Candidate[i]].status = 1;
            // }
            ...
            // }
            // for (int i = expand_stack[expand_stack_size].CandidateSize + 1; i < Candidate.size(); i++) {
            //     g.v[Candidate[i]].status = 2;
            // }
        }
    }
    return ;
