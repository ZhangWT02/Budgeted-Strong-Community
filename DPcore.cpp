...
// g++ -DMOVIELENS -O2 -o DPcore_Movielens DPcore.cpp
// ./DPcore_Movielens 2 5 10
...
char* OUTPUT_FILE = (char*)"Output/Movielens_dp-core.txt";
enum VertexType {
    User,
    Age,
    Occupation,
...
char* OUTPUT_FILE = (char*)"Output/dblp_dp-core.txt";
enum VertexType {
    Author,
    Paper,
    Venues
...
char* OUTPUT_FILE = (char*)"Output/foursquare_dp-core.txt";
enum VertexType {
    User,
    Venues,
    VenuesType
...
char* OUTPUT_FILE = (char*)"Output/amazon_dp-core.txt";
enum VertexType {
    User,
    Product,
    Brand,
...
char* OUTPUT_FILE = (char*)"Output/yago_dp-core.txt";
enum VertexType {
    Person,
    airport,
    imports_wordnet,
...
void DPcore() {
    ...
    if (isValInVector(queryVertexId, Q)) {
        Hsize = 0;
        return;
    }
...
void delVertexAndUpdateQ(int id, vector<int>& Q) {
    ...
    for (int i = 0; i < g.v[id].deg; i++) {
        int neighbor = g.v[id].neighbors[i];
        if (g.v[neighbor].deg == degreeThreshold[g.v[neighbor].type] && !isValInVector(neighbor, Q)) {
            Q.push_back(neighbor);
        }
    }
    VertexType delVertexType = g.v[id].type;
